...
func copy() -> Link:
	...
	"""
	var new_copy = get_script().new(innovation_id,
								weight,
								feed_back,
								from_neuron_id,
								to_neuron_id,
								is_loop_link)
	new_copy.enabled = self.enabled
	return new_copy
	
func print_rep() -> void:
	print(innovation_id, " From: ", from_neuron_id, " to: ", to_neuron_id, " weight: ", weight, " fb:", feed_back, " w:", weight)
