...
@onready var focus_button = get_node(content_path + "/HBoxContainer/Focus")
...
func _ready() -> void:
	...
	#if Parameters.is_highlighter_enabled:
	...
	if not inspected_genome.agent.is_dead:
...
func _on_Focus_button_down() -> void:
	var camera = get_node("/root/Main").get_node("Camera2D")
	
	if is_instance_valid(inspected_genome.agent) and is_instance_valid(inspected_genome.agent.body):
		var pos = inspected_genome.agent.body.global_position
...
func _on_HighlightToggle_toggled(enabled) -> void:
	...
	"""
	if is_instance_valid(inspected_genome.agent):
